---
comments: true
title: "Структуры для общения с голосовым помощником"
---

## Запрос от Webhook'а голосового ассистента к skill'у
-------------------------------------------------------------------------------------------
...
| `interfaces` | `array`  | Интерфейсы, доступные на устройстве пользователя. Сейчас всегда присылается `screen` — пользователь может видеть ответ skill'а (кстати, в перечне такого нет) на экране и открывать ссылки в браузере. |
...
| `skill_id`             | `string`                      | Идентификатор вызываемого skill'а, присвоенный при создании. Соответствует полю «Маруся ID» в настройках skill'а.                                                                                                    |
...
| Поле      | Тип      | Описание                                                                                          |
|-----------|----------|---------------------------------------------------------------------------------------------------|
| `user_id` | `string` | Идентификатор аккаунта пользователя (максимум 64 символа). Уникален в разрезе: «skill + аккаунт». |

### `application`

| Поле               | Тип      | Описание                                                                                                                                                     |
|--------------------|----------|--------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `application_id`   | `string` | Идентификатор экземпляра приложения, в котором пользователь общается с Марусей (максимум 64 символа). Уникален в разрезе: «skill + приложение (устройство)». |
...
```
## Формат ответа обработчика skill'а
-------------------------------------------------------------------------------------------
...
| `payload` | `object` | Любой JSON, который нужно отправить skill'у, если эта кнопка будет нажата, максимум 4 096 байт.                                                                                                                   |
