...
build: bin-dir
	if [ -z "$(shell git status --porcelain)" ]; then \
		sed -i "s|local|$(./workflow/version.sh aplha)|" ./cmd/version.go; \
...
build-linux: bin-dir
	if [ -z "$(shell git status --porcelain)" ]; then \
		sed -i "s|${VERSION}|${VERSION} $(LINUX_OS)/$(ARCH)|" ./cmd/version.go; \
...
build-darwin: bin-dir
	if [ -z "$(shell git status --porcelain)" ]; then \
		sed -i "s|${VERSION}|${VERSION} $(MAC_OS)/$(ARCH)|" ./cmd/version.go; \
...
build-windows: bin-dir
	if [ -z "$(shell git status --porcelain)" ]; then \
		sed -i "s|${VERSION}|${VERSION} $(WINDOWS_OS)/$(ARCH)|" ./cmd/version.go; \
