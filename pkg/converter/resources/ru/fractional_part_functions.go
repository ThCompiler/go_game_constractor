...
func (rs *Russian) ConvertNotLowestScaleToWords(triplet objects.NumericDigitTriplet,
	...
	numberDeclension := getCurrencyFractionalPartDeclension(rs.declension, GetNumberForm(integerPart))
...
// и формирует соответствующие значения (тысячных, миллионных, ...)
func (rs *Russian) ConvertLowestScaleToWords(triplet objects.NumericDigitTriplet,
...
func (rs *Russian) GetZeroAsWordsForFractionalNumber(integerPartTriplets []objects.RuneDigitTriplet) string {
	...
	decl := getCurrencyFractionalPartDeclension(rs.declension, GetNumberForm(integerPartTriplets))
...
func (rs *Russian) GetEndingOfDecimalNumberForFractionalPart(countDigits int, lastDigit objects.Digit) string {
	...
	unitNamePrefix := rs.words.FractionalUnit.FractionalUnitPrefixes[digitIndexInScale]
