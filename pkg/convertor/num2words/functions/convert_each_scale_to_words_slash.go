...
func ConvertEachScaleToWordsSlash(
	...
	if lastScaleWihNumber == 0 {
		convertedResult += convertLastScaleToWordsSlash(
			updatedNumberScalesArray[lastScaleWihNumberIndex].ToNumeric(),
			numberDecl,
			numberScaleForm,
		)
	}
...
func convertOtherScaleToWordsSlash(scaleToConvert objects.NumericDigitTriplet, scaleNumber int, isAloneScale bool,
	...
	if scaleToConvert.Dozens != 1 {
		convertedScale.Dozens = words.WordConstants.N2w.SlashNumberUnitPrefixes.Dozens[scaleToConvert.Dozens]
	...
	if scaleNumber != 0 {
		...
		unitNameEnding := words.WordConstants.N2w.FractionalUnit.FractionalUnitEndings[numberDeclension][numberScaleForm]
